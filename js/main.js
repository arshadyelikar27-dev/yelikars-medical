  ...
  function initLenis() {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
      anchor.addEventListener('click', function (e) {
        ...
        if (targetEl) {
          e.preventDefault();
          const headerHeight = document.getElementById('mainHeader')?.offsetHeight || 72;
  ...
  function initGalleryLightbox() {
    ...
    cards.forEach(card => {
      card.addEventListener('click', () => {
        ...
        const caption = card.getAttribute('data-caption') || card.querySelector('.gallery-title')?.textContent;
        if (img) {
